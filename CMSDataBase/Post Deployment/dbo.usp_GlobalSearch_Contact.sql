...
GO
/****** Object:  StoredProcedure [dbo].[usp_GlobalSearch_Contact]    Script Date: 04-09-2016 16:47:26 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_GlobalSearch_Contact]  --'1214232' 
...
--select * from tx_Claim where ClaimId in
--(select * from SplitString(@cliamId,','))
    

--select * from tx_Account where AccountId in
--(select * from SplitString(@companyId,','))

select * from tx_Contact 
where IsActive = 1 
		AND ( ContactId in (select * from SplitString(@contactId,','))
				OR CompanyId in (select * from SplitString(@companyId,','))
				OR ContactId In (Select Distinct ContactId From tx_Claim Where ClaimId IN (select * from SplitString(@cliamId, ','))) )
